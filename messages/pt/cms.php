<?php
/**
 * Message translations.
 *
 * This file is automatically generated by 'yii message/extract' command.
 * It contains the localizable messages extracted from source code.
 * You may modify this file by translating the extracted messages.
 *
 * Each array element represents the translation (value) of a message (key).
 * If the value is empty, the message is considered as not translated.
 * Messages that no longer need translation will have their translations
 * enclosed between a pair of '@@' marks.
 *
 * Message string can be used with plural forms format. Check i18n section
 * of the guide for details.
 *
 * NOTE: this file must be saved in UTF-8 encoding.
 */
return [
    'Add to section' => '',
    'Added' => '',
    'All Categories' => '',
    'Alt text' => '',
    'Are you sure you want to permanently delete the homepage?' => '',
    'Are you sure you want to permanently delete this entry and its related {n,plural,=1{subentry} other{# subentries}}?' => '',
    'Are you sure you want to remove this entry from the section?' => '',
    'Asset' => '',
    'Asset order changed' => '',
    'Assets' => '',
    'Categories' => '',
    'Category' => '',
    'Category linked to entry.' => '',
    'Category order changed' => '',
    'Category removed from entry.' => '',
    'Change category order' => '',
    'Change entry asset order' => '',
    'Change entry order' => '',
    'Change section asset order' => '',
    'Change section order' => '',
    'Content' => '',
    'Contents' => '',
    'Copy Section' => '',
    'Create Category' => '',
    'Create Entry' => '',
    'Create New Entry' => '',
    'Create new categories' => '',
    'Create new entries' => '',
    'Create new entry assets' => '',
    'Create new section assets' => '',
    'Create new sections' => '',
    'Delete Entry' => '',
    'Delete Homepage' => '',
    'Delete categories' => '',
    'Delete entries' => '',
    'Delete entry assets' => '',
    'Delete section assets' => '',
    'Delete sections' => '',
    'Do you want to duplicate this entry and its {n} subentries?' => '',
    'Duplicate' => '',
    'Entries' => '',
    'Entry' => '',
    'Entry added to section.' => '',
    'Entry order changed' => '',
    'Entry removed from section.' => '',
    'Entry–Category' => '',
    'Link' => '',
    'Link assets' => '',
    'Link entries' => '',
    'Linked entry order changed' => '',
    'Make homepage' => '',
    'Meta description' => '',
    'Meta title' => '',
    'Move / Copy' => '',
    'Move Section' => '',
    'Name' => '',
    'New Category' => '',
    'New Section' => '',
    'Open website' => '',
    'Parent category' => '',
    'Parent entry' => '',
    'Parent entry "{entry}" is not a menu item' => '',
    'Parent status' => '',
    'Please confirm that you want to make this entry the new homepage. This will deactivate the current homepage entry.' => '',
    'Published' => '',
    'Section' => '',
    'Section asset order changed' => '',
    'Section order changed' => '',
    'Section-Entry' => '',
    'Sections' => '',
    'Show in footer' => '',
    'Show in menu' => '',
    'Subcategories' => '',
    'Subentries' => '',
    'The URL "{path}" is protected.' => '',
    'The asset was deleted.' => '',
    'The asset was duplicated.' => '',
    'The asset was updated.' => '',
    'The category was created.' => '',
    'The category was deleted.' => '',
    'The category was updated.' => '',
    'The entry was created.' => '',
    'The entry was deleted.' => '',
    'The entry was duplicated.' => '',
    'The entry was updated.' => '',
    'The section was created.' => '',
    'The section was deleted.' => '',
    'The section was duplicated.' => '',
    'The section was moved.' => '',
    'The section was updated.' => '',
    'The selected entries were updated.' => '',
    'The selected sections were updated.' => '',
    'Title' => '',
    'Update categories' => '',
    'Update entries' => '',
    'Update entry assets' => '',
    'Update entry categories' => '',
    'Update section assets' => '',
    'Update sections' => '',
    'Url' => '',
    'View All Entries' => '',
    'Warning: Deleting this entry cannot be undone. All related sections will also be unrecoverably deleted. Please be certain!' => '',
    'Warning: Deleting this entry cannot be undone. All related subentries and assets will also be unrecoverably deleted. Please be certain!' => '',
    '[ No title ]' => '',
];
